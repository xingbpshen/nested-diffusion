    ...
    if dataset_name in ['ChestXRay', 'ISICSkinCancer']:
        ...
        class CustomTargetTransform:
            def __call__(self, target):
                ...
                target = torch.squeeze(target)  # Squeeze the target tensor to remove dimensions of size 1
                return target
        image_dataset = {'train': PathMNIST(split='train', download=True, root=root_dir,
                                            transform=data_transform['train'],
                                            target_transform=CustomTargetTransform()),
                         'valid': PathMNIST(split='val', download=True, root=root_dir,
                                            transform=data_transform['valid'],
                                            target_transform=CustomTargetTransform()),
                         'test': PathMNIST(split='test', download=True, root=root_dir,
                                           transform=data_transform['test'],
                                           target_transform=CustomTargetTransform())}
    elif dataset_name == 'RotatedMNIST':
        image_dataset = {'train': torchvision.datasets.MNIST(root_dir, download=True, train=True,
                                                             transform=data_transform['train']),
                         'valid': torchvision.datasets.MNIST(root_dir, download=True, train=True,
                                                             transform=data_transform['valid']),
                         'test': torchvision.datasets.MNIST(root_dir, download=True, train=False,
                                                             transform=data_transform['test'])}

    data_loaders = {x: DataLoader(image_dataset[x], batch_size=batch_size,
                                  shuffle=True, num_workers=4) for x in ['train']}

    data_loaders['test'] = DataLoader(image_dataset['test'], batch_size=70,
                                      shuffle=False, num_workers=4, drop_last=True)

    data_loaders['valid'] = DataLoader(image_dataset['valid'], batch_size=70,
                                       shuffle=False, num_workers=4, drop_last=True)
